$(document).ready(function(){
  ...
  var colors = ['#5ae5ba', '#dc94e0', '#d8d1ff', '#6e09ff'];
  var directions = ['+', '-'];
  ...
  if( dotWidth > dotHeight ) {
    ...
    for(var i = 0; i < numRows; i++) {
      ...
      function moveDot() {
        ...
        for( i = 0; i < dots.length; i++ ) {
          ...
          if( dots[i].xMove == '+' ) {
            ...
            drawDot(dots[i])
            if( (dots[i].x + dots[i].radius) >= canvasWidth ) { dots[i].xMove = '-'; }
            if( (dots[i].x - dots[i].radius) <= 0 ) { dots[i].xMove = '+'; }
            if( (dots[i].y + dots[i].radius) >= canvasHeight ) { dots[i].yMove = '-'; }
            if( (dots[i].y - dots[i].radius) <= 0 ) { dots[i].yMove = '+';
          }
        }
        window.requestAnimationFrame(moveDot);
      }
      function drawDot(dot) {
        ...
        context.fillStyle = dot.color; context.fill();
      }
